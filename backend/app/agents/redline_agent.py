...
async def redline_clauses(compared_clauses: List[Dict]) -> List[Dict]:
    """Redline all High/Medium risk clauses concurrently."""
    client = AsyncOpenAI(
    api_key=os.getenv("OPENROUTER_API_KEY"),
    base_url="https://openrouter.ai/api/v1",)
    semaphore = asyncio.Semaphore(3)

    async def bounded_redline(clause):
        async with semaphore:
            await asyncio.sleep(2)
            return await redline_single_clause(client, clause)

    tasks = [bounded_redline(c) for c in compared_clauses]
